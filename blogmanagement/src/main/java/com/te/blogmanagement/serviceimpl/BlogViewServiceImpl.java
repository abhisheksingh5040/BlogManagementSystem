...
import com.te.blogmanagement.repository.BlogPostCommentRepository;
...
public class BlogViewServiceImpl implements BlogViewService {
	...
	// Add comment
	@Override
	public BlogPostComment addCommentToPost(BlogPostCommentDto blogPostCommentDto, Integer postId) {
		BlogPost blogPost = blogPostRepository.findById(postId).get();
		BlogPostComment blogPostComment = new BlogPostComment();
		BeanUtils.copyProperties(blogPostCommentDto, blogPostComment);
		blogPostComment.setBlogPost(blogPost);
		blogPostComment.setPostCommentCreatedAt(LocalDateTime.now());
		blogPostComment.setPostCommentPublishedAt(LocalDateTime.now());
		blogPostCommentRepository.save(blogPostComment);
